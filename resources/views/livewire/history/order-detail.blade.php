<section>
    <div>
        <a wire:navigate href="{{ route('history') }}">
            <button class="button-primary px-3 py-2 mb-3">กลับ</button>
        </a>

        @if($order->status == 'new')
            <button onclick="confirmCancelOrder({{ $order->id }}, 'cancelOrder')" class="button-danger px-3 py-2 mb-3">ยกเลิกคำสั่งซื้อนี้</button>
        @endif
    </div>
    ...
    <div class="row row-gap-3" style="margin-inline: -5px !important;">
        ...
        <div class="col-xl-3 col-md-4 col-sm-6 col-12 px-1">
            <div class="shadow-primary rounded px-3 py-4 bg-white">
                ...
                <p class="m-0">{{ $order->percel_number != NULL ? $order->percel_number : 'รอดำเนินการ' }}</p>
            </div>
        </div>
